...
// Run with:  MAESTRO_LIVE=1 pnpm vitest run packages/cli/test/live-p3.test.ts
...
import { allTools, baseTools } from '@maestro/tools'
import { discoverSkills, SkillRegistry, buildSkillListing } from '@maestro/skills'
...
describe.skipIf(!LIVE)('live phase 3', () => {
  it('loads a skill body that drives a real file write', async () => {
    const dir = await mkdtemp(path.join(tmpdir(), 'maestro-skill-'))
    const skillsRoot = path.join(dir, '.maestro', 'skills', 'make-marker')
    ...
    const registry = new SkillRegistry(await discoverSkills([path.join(dir, '.maestro', 'skills')]))
  ...
  it('runs a subagent (baseTools, fresh context) that does real work', async () => {
    const dir = await mkdtemp(path.join(tmpdir(), 'maestro-subagent-'))
    const provider = new OllamaProvider()
